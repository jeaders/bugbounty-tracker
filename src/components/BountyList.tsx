...
export const BountyList = ({ bounties, filters, onSelect, favorites = [], onToggleFavorite }: Props) => {
  ...
  // Filter con useMemo per evitare ricalcoli inutili
  const filtered = useMemo(() => {
    const q = filters.programma?.toLowerCase() || ''
    return bounties.filter(bounty => {
      ...
      if (q && !bounty.programma.toLowerCase().includes(q)) return false
      return true
    })
  }, [bounties, filters])

  // Paginazione: mostra solo PAGE_SIZE risultati alla volta per performance
  ...
  // Stable handlers per evitare re-render delle card
  const handleSelect = useCallback((b: BugBounty) => onSelect?.(b), [onSelect])
  const handleToggle = useCallback((id: string) => onToggleFavorite?.(id), [onToggleFavorite])

  if (filtered.length === 0) {
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center pb-3 border-b border-cyan-500/30">
          <h2 className="text-2xl font-cyber text-neon-pink neon-glow">BOUNTY DISPONIBILI</h2>
    ...
    <div className="space-y-4">
      ...
      {hasMore && (
        <div className="text-center py-6">
          ...
          >
            CARICA ALTRI ({filtered.length - visible.length} rimanenti)
          </button>
        </div>
      )}
    </div>
...
const BountyCard = memo(({ bounty, isFavorite, onSelect, onToggleFavorite }: BountyCardProps) => {
  const handleClick = useCallback(() => onSelect?.(bounty), [bounty, onSelect])
  const handleToggleClick = useCallback((e: ReactMouseEvent<HTMLButtonElement>) => {
    e.stopPropagation()
    onToggleFavorite?.(bounty.id)
  }, [bounty.id, onToggleFavorite])
    ...
    >
      <div className="grid md:grid-cols-4 gap-4">
        <div className="md:col-span-3">
          ...
          <p className="text-sm text-cyan-600 mt-2 leading-relaxed line-clamp-2">{bounty.descrizione}</p>

          <div className="flex flex-wrap gap-2 mt-4">
            <span className={`cyber-badge ${SEVERITY_COLORS[bounty.severita] || SEVERITY_COLORS.medium}`}>
